...
public class BackEndDijkstra<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    if (!this.containsVertex(start) || !this.containsVertex(end)) {
      ...
      // checks to see whether the vertice(s) exists.
      throw new NoSuchElementException("Sorry! The specified vertex does not exist.");
    }

    PriorityQueue<Path> dp = new PriorityQueue<Path>();

    Hashtable<T, Path> strp = new Hashtable<>();

    Vertex startV = vertices.get(start);
    Vertex endV = vertices.get(end);

    Path newp = new Path(startV);
    Path temp = null;
    ...
    while (!dp.isEmpty()) {
      newp = dp.remove();
      Iterator<Edge> repeat = newp.end.edgesLeaving.iterator();
      
      while (repeat.hasNext()) {
        Edge edge = repeat.next();
        temp = new Path(newp, edge);

        if (strp.containsKey(edge.target.data)) {
          
          //Checks to see whether new edge is shorter. 
          if (strp.get(edge.target.data).compareTo(temp) > 0) {
            strp.put(edge.target.data, temp);
            dp.add(temp);
          }
        } 
        else {
    ...
    // NoSuchElementException thrown if path from start to end does not exist.
    if (strp.get(end) == null) {
      throw new NoSuchElementException("Path between chosen vertices does not exist");
    }
  ...
  public List<T> shortestPath(T start, T end) {
    ...
    Path wistful = dijkstrasShortestPath(start, end);
    if (!this.containsVertex(start) || !this.containsVertex(end)) {
      throw new NoSuchElementException("Sorry! Specified vertex does not exist.");
    }

    if (wistful == null) {
      throw new NoSuchElementException("No path(s) between the verticies were not found.");
    } else {
      return wistful.dataSequence;
    }
  }
