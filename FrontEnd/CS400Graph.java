...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path copyPath, Edge extendBy) {
			...
			List<T> newDataSequence = new LinkedList<>();
			for (int i = 0; i < copyPath.dataSequence.size(); i++) {
				newDataSequence.add(copyPath.dataSequence.get(i));
			}
			this.dataSequence = newDataSequence;
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		if (!this.containsVertex(start) || !this.containsVertex(end)) {// checks if vertices exist
		...
		List<Path> unfinished = new ArrayList<Path>();
		List<Path> noPathThere = new ArrayList<Path>();
		Path p = new Path(startVertex);
		if (startVertex.edgesLeaving.size() == 0) {
			throw new NoSuchElementException("No paths leaving from start vertex.");
		}
		unfinished.add(p); // adds starting path to unfinished list

		while (unfinished.size() != 0) {
			for (int i = 0; i < unfinished.size(); i++) { // iterates through the unfinished paths list
				
				//Case 1: When the final node in the path is equal to the target vertex
				if (unfinished.get(i).end.equals(endVertex)) {
					pq.add(unfinished.get(i));
					unfinished.remove(i);
				}

				//Case 2: When the final node in the path is not the target vertex and the final node has leaving edges
				else if (!unfinished.get(i).end.equals(endVertex) && unfinished.get(i).end.edgesLeaving.size() != 0) {
					for (int j = 0; j < unfinished.get(i).end.edgesLeaving.size(); j++) { // iterates through leaving edges
						
						//Checks if a leaving edge returns to the starting Vertex (loop)
						if (unfinished.get(i).end.edgesLeaving.get(j).target.equals(startVertex)) {
					    
						//Checks if the leaving edge returns to itself (loop)
						} else if (unfinished.get(i).end.edgesLeaving.get(j).target.equals(unfinished.get(i).end)) {
						
						//If the leaving edge goes to a valid vertex, updates the path object and adds to the
						//unfinished list
						} else {
							Path toAdd = new Path(unfinished.get(i), unfinished.get(i).end.edgesLeaving.get(j));
							Object toCheck = toAdd.dataSequence.get(toAdd.dataSequence.size() - 1);
							int counter = 0;
							
							//Checks if the updated path is looping through vertices besides the start and current vertices
							for (int k = 0; k < toAdd.dataSequence.size(); k++) {
								if (toCheck.equals(toAdd.dataSequence.get(k))) {
									counter += 1;
								}
							}
							if (counter <= 2) {
								unfinished.add(toAdd);
							}
						}
					}

					unfinished.remove(i); // removes the path after updated paths have been created
					
				  //Case 4: if the final node in the path has no leaving edges
				} else if (!unfinished.get(i).end.equals(endVertex) && unfinished.get(i).end.edgesLeaving.size() == 0) {
					noPathThere.add(unfinished.get(i));
					unfinished.remove(i);
				}
			}
		...
		if (pq.isEmpty()) { //Checks if any successful paths were found
			throw new NoSuchElementException("No path between those two nodes.");
		} else {

			if (allFoundEnd(pq, endVertex) && !pq.isEmpty()) {
				Iterator<Path> it = pq.iterator();
				shortestPath = it.next();
				while (it.hasNext()) { //confirms that the shortestPath object is indeed the shortest path in the list
					Path nextPath = it.next();
					if (nextPath.distance < shortestPath.distance) {
	...
	/*
	 * This confirms that all of the paths in the successful paths Priority Queue do
	 * in fact reach the target vertex.
	 */
	protected boolean allFoundEnd(PriorityQueue<Path> pq, Vertex end) {
		Iterator<Path> it = pq.iterator();

		while (it.hasNext()) {
			if (!it.next().end.equals(end)) {
				return false;
			}
		}
		return true;
	...
	public List<T> shortestPath(T start, T end) {
		...
		Path l = dijkstrasShortestPath(start, end);
		if (l == null) {
			throw new NoSuchElementException("No path between those two vertices can be found.");
		} else {
			return l.dataSequence;
		}
	}
